...
# 参数优化的超参数
NUM_EPOCHS = 30  # The number of epochs of training 所有数据的训练大轮次数，每一大轮会对所有的数据进行训练
BATCH_SIZE = 128  # the batch size 每一轮中的每一小轮训练128个样本
LEARNING_RATE = 0.04  # the initial learning rate #学习率

LOG_FILE = "word-embedding.log"
data = LineSentence('./data2/zhwiki.data.txt')
allWords = [word for line in data for word in line]

wordCount = Counter(allWords)  # 统计词频

def getWordEmbedding(embeddingSize, path):
    """
    取出预训练好的word2vec中的词向量
    """
    vocab = []
    wordEmbedding = []
    vec = gensim.models.KeyedVectors.load_word2vec_format(path, binary=False)
    vocab.append('PAD')
    vocab.append('UNK')
...
def genEmbedding(embeddingSize, path):
    vocab, wordEmbedding = getWordEmbedding(embeddingSize, path)
...
# genEmbedding(EMBEDDING_SIZE, './vector/zhwiki_CBOW.model.vector')
    ...
    for i, (input_labels, pos_labels, neg_labels) in enumerate(dataloader):
        ...
        if i % 1000 == 0:
            with open(LOG_FILE, "a") as fout:
                fout.write("epoch: {}, iter: {}, loss: {}\n".format(e, i/1000, loss.item()))
                print("epoch: {}, iter: {}, loss: {}".format(e, i/1000, loss.item()))


embedding_weights = model.input_embeddings()
np.save("./vector/new_attn_CBOW_embedding.npy".format(EMBEDDING_SIZE), embedding_weights)
# torch.save(model.state_dict(), "data/attn_embedding.th".format(EMBEDDING_SIZE))

# evaluate trained model
for word in ["自然", "数学", "孔子", "飞机", "茶叶", "西装", "青椒", "客栈", "老师", "鼠标"]:
